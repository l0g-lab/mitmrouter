...
cleanup() {
    ...
    ip link set dev $in_interface down
    ip addr flush dev $in_interface
...
clean_setup_ip() {
    ...
    if [[ "$mode" == 'wireless' ]]; then
        ...
        ip addr flush dev $in_interface
        # Setup
        ip link set dev $in_interface up
        ip addr add $LAN_IP/24 dev $in_interface
        sleep 1
    elif [[ "$mode" == 'wired' ]]; then
        # Clean
        ip link set $in_interface down
        ip addr flush dev $in_interface
...
build_run_dhcpd() {
    ...
    cp $DHCPD_CONF /etc/dhcp/dhcpd.conf
    sed -i "/INTERFACESv4/ s/.*/INTERFACESv4=$in_interface/" /etc/default/isc-dhcp-server
...
clean_setup_firewall() {
    ...
    #iptables -t nat -A PREROUTING -i $in_interface -p tcp -d 1.2.3.4 --dport 443 -j REDIRECT --to-ports 8080
}
